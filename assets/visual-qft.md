...
This group is generated by the operation of adding $1$, aka the
"successor" or "shift" function $S: |k\rangle \mapsto
|k+1\rangle$ modulo $d$.
This corresponds to a unitary matrix acting on the Hilbert space $\mathcal{H}_d$:
...
The first thing to note is that $S^d = \mathbb{I}$ is the identity,
...
and hence $\alpha_k = \omega\alpha_{k+1}$.
