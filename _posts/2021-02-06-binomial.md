...
Note that we can use property (\ref{e}) to *define* the exponential, since if
...
$$
(1 + x)^{qn} = (1 + x)^{m} \approx 1 + mx \tag{2}\label{m}
$$
...
$$
(1 + x)^n = 1 + nx + \frac{n(n-1)}{2} x^2 + O(x^3), \tag{3} \label{second}
$$
