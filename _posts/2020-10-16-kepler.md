...
=\frac{1}{u_0\sqrt{1-A^2u_0^{-1}}}, \quad b = \sqrt{r_\text{max}
r_{\min}} = \frac{\sqrt{1-A^2u_0^{-1}}}{u_0},
$$

with $r_\text{max} = 1/u_\text{min}$ $r_\text{max} = 1/u_\text{max}$.
Rather than derive these, let's keep this post relatively short and take these formulas as given.
...
The semi-major and semi-minor axis are related by the semi-lattice
rectum $p = u_0^{-1}$ by $ap = b^2$, and hence

$$
T \propto p^{-1/2}ab = a^{3/2},
$$
