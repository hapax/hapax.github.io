...
$E = mc^2$ and gravity), anad hence microscopes make black holes.
...
In fact, the messenger particle is a photon, so in order
...
This sum need not be less than $1$, but really, this is a very loose
order-of-magnitude estimate for the probability $P$, and other constants
can appear out the front, so $P \approx C\alpha/(1-\alpha)$.
But if $\alpha \to 1$, $P$ blows up to infinity, and no constants out
...
$k_e e^2$.
[Technically, we usually imagine electrons to have perfectly well-defined momenta,
$\Delta p= 0$, so by Heisenberg's principle, $\Delta x = \infty$. They could be anywhere!
So $r^2$ really isn't relevant.]
...
that happen.
Since we're doing quantum mechanics $\hbar$ is involved ($[\hbar] =
(ML^2/T^2)T$), and since photons are involved we can use the speed of
light $c$ ($[c] = L/T$).
...
answer, and unitarity breaks down.
...
If they are the *rest masses*, it is true for subatomic particles, as
...
This is the energy required by a microscope probing the Planck length.
...
energies of $E \sim 10^{-6} \text{ J}$, which is $15$ orders of
magnitude away!
