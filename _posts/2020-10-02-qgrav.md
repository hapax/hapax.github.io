...
**October 2, 2020.** *People often say that quantum gravity is hard
  ...
  other statements, such as diffeomorphism invariance and local
  observables.*
...
(I like to imagine a bunch of blind soccer players communicating their
positions by throwing baseballs to each other.)

Suppose there is some background probability $R$ for this repulsive
process to occur.
Given that it does, let's assume the probability of exchanging a
single photon is $\alpha$.
...
is *unitary*.
Otherwise, unitarity breaks down and predicts nonsense results.

To check that quantum electrodynamics is unitary, we need to find
$\alpha$.
Let's recall some facts about electromagnetism.
*Couloumb's law* for repulsion is
...
For our simple model, we'll assume that the separation governs the
probability $R$, but not the number of photons exchanged.
Then the likelihood of exchanging a single photon would seem to be
governed by $k_e e^2$.
This is not a number, so to make it dimensionless, we use the
technique of dimensional analysis with fundamental constants.
Since quantum mechanics is involved we use
$\hbar$, and since photons are involved we use $c$, the speed they
travel at.
...
unitary.
Of course, we are completely butchering quantum electrodynamics, but
the gist is right.
...
To find $g$, we throw away the $r^2$, and divide by $\hbar
c$, where the inclusion of $c$ is justified because gravitons are
massless and travel at the speed of light.
...
sensible, unitary answer right?
The subtlety lies in how we interpret the masses $M_1$ and $M_2$.
If they are the *rest masses*, it would seem to be true.
But gravity doesn't care about these!
It care about the *relativistic mass* $M = E/c^2$, which is the same
idea we used above for photons and black holes.
...
If we crank up the energies until $g \approx 1$, our graviton exchange
theory becomes *non-unitary* and stops making sense.
...
where $E_P = \hbar c/\ell_P$ is the *Planck energy*, the energy
required by a microscope probing the Planck length.
Note that, numerically, the Planck energy is $E_P \approx 2 \times
10^9 \text{ J}$.
Accelerating a subatomic particles to these sorts of energies is out
of the question for the forseeable future.
The Large Hadron Collider, for instance, accelerates protons to
energies of $E \sim 10^{-6} \text{ J}$, around $15$ orders of
magnitude away.
...
stupid question and it simply doesn't make sense to ask questions
about these energies (loop quantum gravity).
