...
linear approximations and the Wiener process, or Brownian motion, we
just introduced.
...
interest rate (called the *risk-free rate*).
For continuous compounding, if the value of the stock is $y$, the
value $y'$ a short time $t$ later is
...
The value will also fluctuate random by an amount $\sigma\, dW$, where
$dW$ represents random, unit variance step, and $\sigma \, dW$ has
variance $\sigma^2$.
