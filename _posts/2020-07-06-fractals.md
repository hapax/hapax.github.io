...
mathjax: true
comments: true
title:  "From fractals to finance"
...
This *additivity property* is somewhat rare.
...
$mh$.
Note that this satisfies the additivity constraint, since

$$
\Delta \hat{F}(k) + \Delta \hat{F}(h-k) = m [k + (h-k)] + \sigma
\mathcal{N}(0, k) + \sigma \mathcal{N}(0, h - k) + o(h) \sim \Delta \hat{F}(h).
$$
...
I don't have space to expand on the details, but this equation won
...
jagged, taking us to fractals and finance!
