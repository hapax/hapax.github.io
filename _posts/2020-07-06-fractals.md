...
Recall that local linear approximations were self-similar when we use
the same scale for $x$ and $y$, while Brownian motion requires us to
scale them differently.
If we no longer require our local approximation to have uniform
self-similiarity, we can simply smush the two together:
...
combining ordinary local linear approximations and random fractals may
no longer have the self-similarity properties that we initially
demanded, but remain useful nevertheless.

You might ask why we're allowed to drop self-similarity, since that
seemed to be the point of all this business of fractals and infinite
zoom in the first place.
The introduction of inhomogenous zoom indicate that zooming in can
actually be achieved in different ways.
All we've done is take some sensible objects for local approximation
and plonk them together.
The local approximation will still make sense, even though the
combination is no longer self-similar!
