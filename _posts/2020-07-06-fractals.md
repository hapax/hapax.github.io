...
**July 6, 2020.** *A derivative is a local linear
  ...
  stochastic derivatives, here interpreted as random fractal approximations, and
...
#### Varieties of self-similar experience*
...
approximations which are not fixed points of some zoom operator?
The answer is yes.
We can then combine local approximations with different scaling
behaviours willy-nilly.
For instance, we can comine a local linear approximation with Brownian
motion:
...
random Gaussian step with zero mean, there is a constant 
