...
space-filling tessellations by a single polyhedron, in the same way that the <a
href="#sec-4-2">hexagonal tiling</a> fills the plane.
We will start our journey into the properties of infinite foams by
consider *regular polyhedra*, aka [the Platonic solids](https://en.wikipedia.org/wiki/Platonic_solid).

As a warm-up, let's start with the plane.
This can be tiled in three ways by regular polygons: squares, triangles, and hexagons.
Let's see why these are are the only possibilities, and which of these
could describe a two-dimensional foam.

<span style="padding-left: 20px; display:block">
(a) Prove that internal angle of a regular $d$-gon is
</span>
...
<span style="padding-left: 20px; display:block">
(b) Explain why we must have $2\pi = n\theta$ for some whole number
$n$.
...
<span style="padding-left: 20px; display:block">
(c) Show that only the hexagonal lattice is locally minimal, as
discussed <a href="#sec-4-2">above</a>.
</span>

Let's upgrade to three dimensions.
In this case, there are only five regular polyhedra:
cube,
tetrahedron, octahedron, dodecahedron and icosahedron.
Now there are two types of relevant angles:
[*solid angles*](https://en.wikipedia.org/wiki/Solid_angle) at a
vertex, and
[*dihedral angles*](https://en.wikipedia.org/wiki/Dihedral_angle) at
an edge.
They are listed
[here](https://en.wikipedia.org/wiki/Platonic_solid#Angles).

Let's focus on what happens a vertex.
First of all, the total solid angle to fill around the vertex is
$4\pi$ in "solid radians" or steradians.
(This is the surface area of a unit sphere, analogous to the perimeter
of a unit circle we use to measure radians.)
The solid angles at a vertex are given by

\begin{align*}
\Omega_\text{cube} & = \frac{\pi}{2}\\
...
The vertex could meet other vertices, an edge, or a face, but for
simplicity, we'll assume "vertex only" tessellations.

<span style="padding-left: 20px; display:block">
(d) Argue that, for a Platonic solid to tessellate "vertex only", it
must have vertex angle $\Omega = 4\pi/n$ for some integer $n$.
</span>

<span style="padding-left: 20px; display:block">
(e) Using a calculator or symbolic algebra package, show that only
$\Omega_\text{cube}$ can be written in the form $2\pi/n$.
Conclude that the cube alone can fill space (with vertices touching).
</span>

Even if we allow the other solids to slide around, so that vertices
touch edges or faces, we can check all the possible combinations and
show that the cube remains the only way to tessellate space with a
regular polyhedron.
Let's see if it could possibly describe a foam!

<span style="padding-left: 20px; display:block">
(f) Suppose we tessellate space using a cell with $f$ faces, each with
$e$ edges.
...
<span style="padding-left: 20px; display:block">
(f) Showing using Exercise 20 or Plateau's laws directly that an
infinite foam cannot be cubic.
</span>
...
*Exercise 22.*
Let's expand our horizons beyond Platonic solids to the more general
...
We saw in the previous exercise that, even for the simplest shapes,
working out which ones tessellate takes work, so for these more
complicated solids, we will skip straight to the answer.
...
Our little catalogue of tessellations suggests that our options
...
by $0.3 \%$, the tinest of margins!
It is unknown whether this is optimal.
