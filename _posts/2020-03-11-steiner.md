...
should be locally minimal, just like a two-dimensional foam, with
walls following the $120^\circ$ rule.
...
This seems like the most efficient way to build walls to separate
cells of equal size, a hypothesis called the [honeycomb conjecture](https://en.wikipedia.org/wiki/Honeycomb_conjecture).
But it turns out to be extremely hard to prove, since
you need to somehow check all the irregular tilings as well.
The honeycomb conjecture became the honeycomb *theorem* in 1999, when
it was proved by
[Thomas Hales](https://en.wikipedia.org/wiki/Thomas_Callister_Hales),
famous for his work on the related problem of sphere packing.
...
### 4.3. Plateau's laws<a id="sec-4-3" name="sec-4-3"></a>
...
for the cross-section, despite the previous exercise.
...
In the <a href="#sec-4-2">last section</a>, we were able to hack two-dimensional foams by
...
This is the computational model for most applications of minimal
...
that nature opts for this solution.
It turns out that the sphere is not only locally minimal, but the
*globally* minimal surface enclosing a fixed volume, a deep
mathematical result called the *isoperimetric inequality*.
...
A truncated octahedron has $E = 36$ edges and $F = 14$ edges, made up of $8$ hexagons and
$6$ squares.
...
three-dimensional version of the hexagonal lattice, and hence what
four-dimensional bees would store their honey in!
...
faces of a bubble, and cannot pin them down further.
...
But Murphy's law applies to mathematics as well as life: whatever can
...
deeply interrelated, and those interrelations deserve our careful attention.
...
Perhaps there is more in *playing* with them than we generally
imagine as well.
