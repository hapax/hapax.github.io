...
Even if we allow solids to slide around, so that vertices
touch edges or faces, a proof by cases shows that the cube remains the
only way to fill space by spamming a Platonic solid.
