...
4. <a href="#sec-4">Soap bubbles</a>
   ...
   4. <a href="#sec-4-4">Plateau's laws</a>
...
In this short section, we make a couple of further observations about
hexagonality. The section is optional, and slightly more advanced, as
...
used.
Darwin discusses the hive-making instinct and its relation to evolutionary fitness in his
...
For honeycomb walls to be locally minimal, then like a two-dimensional
foam they must obey the $120^\circ$ rule.
If we also require our honeycomb cells to be equal size, then a
natural guess is the hexagonal lattice, and indeed, this is the only
...
states the hexagonal lattice is the *globally* minimal way to separate
cells of equal size.
...
finally gave a [formal proof](https://arxiv.org/pdf/math/9906042.pdf).
We will consider

<figure>
    <div style="text-align:center"><img src
    ="/images/posts/bees.jpg" width="60%"/>
...
lattice <a href="#sec-4-4">below</a>.

Our second observation concerns the scaling assumptions we made in the
previous section.
We assumed that if a two-dimensional foam has size $\sim L$,
...
But the foam could live on a curved surface, such as a saddle or a sphere.
...
<figure>
    <div style="text-align:center"><img src
    ="/images/posts/heptagon.png" width="60%"/>
		    <figcaption><i>A hyperbolic foam made of heptagons. Parcly
    Taxel, Wikimedia Commons.</i></figcaption>
	</div>
	</figure>

You might wonder what bees living in hyperbolic space store their
honey in.
The question of the globally minimal arrangement is made trickier
because the weird properties of the saddle mean the answer changes
with the area of a cell!
But in November 2019, a group of undergraduates
[proved that](https://arxiv.org/pdf/1911.04476.pdf) proved that if
hyperbolic bees wish to store their honey in compartments of size
$\pi/3$, the heptagonal tiling is best!
At least, it is optimal among *regular* tilings, but the "hyperbolic
honeycomb conjecture" that this is optimal among all tilings,
including the irregular ones, remains open.
...
### 4.4. Plateau's laws<a id="sec-4-4" name="sec-4-4"></a>
...
We will do this in the form of an extended exercise.
...
Constant mean curvature implies that any (straight) slice
through the surface resembles a straight line or the arc of a circle.
Let's now take a cross-section $A$ which "cuts
through" every face and every Plateau border of the foam, forming a
network where edges correspond to bubble faces and points to Plateau borders.
...
<span style="padding-left: 20px; display:block">
(g) If four borders meet at a vertex, what is the most symmetric arrangement?
...
borders meet at a vertex, separated by the angle $\theta_4
\approx 109.5^\circ$.
</span>
...
In fact, generalised bubbles blowers are used for most applications of minimal
...
You can explore two fixed volume problems below: two bubbles of equal
volume in Exercise 17, and an *infinite* number of equal volume bubbles
in Exercise 18.
...
*Exercise 18.* Using similar techniques to our <a
...
We can also use these techniques to learn about *infinite* foams!
...
<span style="padding-left: 20px; display:block">
(g) The fact that we've introduced *average* quantities means we can
actually talk about infinite foams!
Suppose we have an infinite number of bubbles of equal volume.
Show that they will *not* arrange themselves into a space-filling
tessellation by
[rhombic dodecahedra](https://en.wikipedia.org/wiki/Rhombic_dodecahedral_honeycomb),
polyhedra with $f = 12$ faces and $e = 24$ edges.
*Hint.* In an infinite tessellation, $F_\text{avg} = f$ and
$E_\text{avg} = e$.
</span>

<span style="padding-left: 20px; display:block">
(h) The *Kelvin structure* is a space-filling tessellation made from
[truncated octahedra](https://en.wikipedia.org/wiki/Truncated_octahedron).
A truncated octahedron has $e = 36$ edges and $f = 14$ faces, made up of $8$ hexagons and
$6$ squares.
...
In 1887, [Lord Kelvin](https://en.wikipedia.org/wiki/William_Thomson,_1st_Baron_Kelvin)
(1824-1907) conjectured that his eponymous tessellation was the most efficient way to separate
an infinite number of cells of equal volume.
In other words, he thought the Kelvin structure was the
three-dimensional version of the hexagonal lattice, or more
whimsically, the structure four-dimensional bees would store their honey in!
This seems reasonable: of all the space-filling tessellations with
regular polyhedra, the only three with equal volume cells are the
cubic, rhombic dodecahedral, and Kelvin structure.
The first two are ruled out because they do not obey $3F_\text{avg} -
E_\text{avg} = 6$.

But this is not a proof, and there is reason to be cautious.
When we studied two-dimensional foams <a href="#sec-4-1">earlier</a>,
we were able to pin down the average
number of sides of a face.
Here, the best we can do is relate the average number of edges and
faces of a bubble, and cannot push our method further.
This is not a weakness of our method; there are just more possibilities in three dimensions!
So, maybe there is some weird, irregular tessellation out there,
better than the Kelvin structure?
Understandably, this possibility never entered Kelvin's head.

Murphy's law applies to mathematics as well as life: whatever can
go wrong, will go wrong.
In 1993, [Weaire and Phelan](https://en.wikipedia.org/wiki/Weaire%E2%80%93Phelan_structure) realised that an odd bird made from alternating
non-uniform polyhedra of equal volume needs less film per cell than the Kelvin structure.
And like blowing bubbles to check the isoperimetric inequality,
you can reproduce this structure from careful soap film experiments!
This is yet more proof that physics, mathematics, and computation are
deeply interrelated, and those interrelations deserve careful attention.
...
We have seen there is more in *playing* with them than generally
imagined as well.

#### Acknowledgments

I'd like to acknowledge Rafael Haenel, Pedro Lopes and Haris Amiri for
...
- [*On The Origin of Species*](https://www.gutenberg.org/files/1228/1228-h/1228-h.htm#link2H_4_0010)
-(1859), Charles Darwin.
