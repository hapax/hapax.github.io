...
**March 11, 2020.** *A self-contained introduction to minimal
...
We'll step through the reasoning slowly, starting with case of three cities.

*Applications.* Steiner trees in the plane, and other closely related variants, pop in
real-world applications everywhere, from railways to
[computer chip design](https://en.wikipedia.org/wiki/Very_Large_Scale_Integration), and
[telecommunications networks](https://homepage.univie.ac.at/ivana.ljubic/research/STP/)
to the
[classification of life itself](http://www.math.ucsd.edu/~fan/ron/papers/82_01_phylogeny.pdf).
Some of these applications require a variant of our Steiner problem,
but many insights carry over.
We are doing real-world mathematics.
...
Roughly, it means: for $n$ fixed nodes, the running time $t(n)$ for
the best algorithm grows faster than any *polynomial* in $n$.
More precisely, pick any polynomial $p(n)$ of degree $k$.
Then, as $n$ gets large, only the degree $k$ term matters.
We can write:

$$
t(n) \gtrsim \alpha n^k \approx p(n),
$$

for *any* $k$, where $\gtrsim$ means "for all suitably large $n$".
This is consistent with the (slightly shifty) argument from the
exponential number of tinkertoys.

The technical term for "effectively impossible to compute" is
[**NP-hard**](https://en.wikipedia.org/wiki/NP-hardness), meaning
roughly that computer scientists are pretty sure there is no quick way
to find the exact minimal network.
It will always take a ridonkulously long time!
...
But although exactly solving Steiner trees is crazily difficult, it
...
This lets us snatch victory from the jaws of computatonal defeat.
...
Despite being named after him, Steiner trees solve a very different
problem! The minimal network, and Fermat's problem, are only the same
for three points.

Fermat, Torricelli, and Steiner were only interested in geometry.
200 years later, the great German mathematician
[Carl Friedrich Gauss](https://en.wikipedia.org/wiki/Carl_Friedrich_Gauss)
(1777 -- 1855) considered the optimisation problem of designing a
minimal rail network between four German cities.
(You can learn more about this in Exercise 15, at the end of this
tutorial, and even solve it experimentally!)
But Gauss apparently lost interest, only sketching the answer for
different configurations of four cities.
The $n$ city problem we've been considering here was not posed until
1934 by Jarnik and Kossler.
The Jarnik-Kossler problem was later popularised under the name
"Steiner tree" in Courant and Robbins' classic 1941 text, *What is mathematics?*.
...
To quote Rolf Landauer,

<span style="padding-left: 20px; display:block">
Information is physical.
</span>

I will take up this fun theme elsewhere, but for our purposes,
let's just consider the traditional division of computers into hardware and software.
...
Right now, we are seeing the advent of *quantum computing*, with
...
*Exercise 15 (trains and soap bubbles).* As advertised <a
