...
4. <a href="#sec-4">Soap bubbles</a>
   ...
   2. <a href="#sec-4-2">Computing with bubbles</a>
   3. <a href="#sec-4-3">Plateau's laws (bonus)</a>
...
We can snatch victory from the jaws of computatonal defeat!
...
You can learn more about this in Exercise 16, at the end of this
tutorial, and even find the minimal network "experimentally".
...
effortlessly as a child drops a ball to the ground.
...
This is related to the fact that bees build honeycombs in a hexagonal
lattice.
They want to use as little wax as possible, so the honeycomb walls
should be locally minimal, just like a two-dimensional foam.
It follows that the walls will follow the $120^\circ$ rule.
If the walls are required to partition cells of equal size, it seems
reasonable to guess that a hexagonal lattice is best.
[This guess](https://en.wikipedia.org/wiki/Honeycomb_conjecture) --- although very natural --- turns out to be extremely
hard to prove mathematically, and was only verified in 1999 by
[Thomas Hales](https://en.wikipedia.org/wiki/Thomas_Callister_Hales)
(famous for his work on sphere packing).
...
### 4.2. Computing with bubbles <a id="sec-4-2" name="sec-4-2"></a>
...
in which black holes, DNA, and animal brains are all special-purpose
...
### 4.3. Plateau's laws (bonus)<a id="sec-4-3" name="sec-4-3"></a>

*Edit: Section added April 8, 2020. Pictures to come!*

We've described the rules obeyed by
two-dimensional foams.
We will end this post with a hefty bonus exercise on the behaviour of their
*three-dimensional* counterparts.

A two-dimensional foam has cells (areas) separated by a locally
minimal network.
In three dimensions, bubble cells are volumes separated by a locally
minimal surface.
Locally minimal networks, such as foams, obey the $120^\circ$ rule.
A locally minimal surface obeys a generalisation of this rule, encoded
in *Plateau's laws*.
These are named after the Belgian mathematician
[Joseph Plateau](https://en.wikipedia.org/wiki/Joseph_Plateau)
(1801-1883) who arrived at them by assiduously observing soap
bubbles!
Plateau's laws are connected to some deep mathematical problems, and
although being posed in the 19th century, they weren't proved until
1976!

Here are the first three laws:

1. The faces in a soap film are smooth, i.e. no kinks.
2. Any face has constant mean curvature.
3. Faces always meet three at a time, separated by $120^\circ$, at a
   shared *Plateau border*.

Constant mean curvature just means that any (straight) slice
through the surface looks like a straight line or the arc of a circle.
Let's now take a cross-section $A$ of a three-dimensional foam which "cuts
through" every face and every Plateau border, forming a network with
edges corresponding to faces and points to Plateau borders.
...
<span style="padding-left: 20px; display:block">
(c) Explain why <a
href="#sec-4-1">our earlier result</a> $D_\text{hex} = 6 + E_b$ holds
for the cross-section, despite (b).
Under the same scaling assumptions, faces in $A$ tend to be hexagonal.
</span>
...
<span style="padding-left: 20px; display:block">
(d) Given Plateau's first law, explain why at least three edges meet
at any point in $A$.
</span>
...
symmetrically.
The principles lead directly to Plateau's fourth law for vertices
where Plateau borders meet in a three-dimensional foam.
(From now on, "vertices" refers to the foam, and not the cross-section.)

<span style="padding-left: 20px; display:block">
(f) From Plateau's first law, explain why at least four Plateau
borders meet at a vertex.
</span>

<span style="padding-left: 20px; display:block">
(g) Assume four Plateau borders meet at a vertex.
What is the most symmetric arrangement of borders?
	*Hint.* Think about methane.
	</span>

<span style="padding-left: 20px; display:block">
(h) Show that, for part (g), the borders are separated
by angle $\theta_4 = \cos^{-1}(-1/3)$.
</span>

This is indeed Plateau's fourth law:

<span style="padding-left: 10px; display:block">4. Plateau borders meet four at a vertex, separated by the (tetrahedral) angle $\theta_4
\approx 109.5^\circ$.
</span>

In the last section, we were able to hack two-dimensional foams and by
adding screws as fixed points.
Similarly, we can hack a three-dimensional foam by adding *fixed
boundaries* and immersing in soapy water.
In other words, a bubble blower is a kind of computer too!
Instead of fixing a boundary, we can also fix the *volume* of a bubble,
i.e. that it enclose a given volume of air.

The simplest example is a single bubble.
The only smooth, constant mean curvature surface enclosing a finite
volume is the sphere (this is not mathematically easy to show, but is intuitive).
We can add extra bubbles to the sphere (increasing the radius slightly
to ensure it has fixed volume) but it turns out these always increase
the surface area.
Thus, the simple spheres preferred by nature are not only locally
minimal, but *globally* minimal.
In slightly more formal language, the minimal area surface enclosing a
given volume is the sphere. In mathematics, result is called the *isoperimetric
inequality*, and while it is hard to prove, you compute the correct
answer once your film leaves the bubble blower, containing precisely
the volume of air you blew into it.
You can explore two bubbles of equal volume in Exercise 17, and an
*infnite* number of equal volume cells in Exercise 18.

---

*Exercise 17.* Consider two bubbles of fixed volume $V$.
	Assuming that extra bubbles increase the surface area, use
	Plateau's law to determine the shape of the minimal surface.
	(You may also assume that face between bubbles is flat.)
	Mathematicians didn't prove this configuration was globally
	minimal until 2002!
...
*Exercise 18.* Using similar reasoning to <a href="#sec-4-1">our earlier
results for two-dimensional foams</a>, we can show that most faces in
a three-dimensional foam have *less* than six sides.
We will 
...
Why is this consistent with the result that $D_\text{hex} = 6 + E_b$
for cross-sections $A$ of the foam?
</span>

<span style="padding-left: 20px; display:block">
(g) The *Kelvin structure* is a space-filling tessellation made from
[truncated octahedra](https://en.wikipedia.org/wiki/Truncated_octahedron),
with $E = 36$ edges and $F = 14$ edges, made up of $8$ hexagons and
$6$ squares.
Check these properties are consistent with (e) and (f).
</span>

The Kelvin structure is a beautiful tiling of space consistent with
Plateau's laws. It is named after [Lord Kelvin](https://en.wikipedia.org/wiki/William_Thomson,_1st_Baron_Kelvin)
(1824-1907), who conjectured it was the minimal area way to separate
an infinite number of cells of equal volume, a three-dimensional
analogue of the hexagonal lattice.
But, unlike the two-dimensional foam, the results in this exercise
do not let draw conclusion about the average number of faces of a
cell, just the relation between edges and faces.
This is connected to the fact that there are many more possibilities
for foams in three dimensions!

And, as discovered in 1993, the Kelvin structure is *not* the global
minimum: it is beaten by
[Weaire-Phelan structure](https://en.wikipedia.org/wiki/Weaire%E2%80%93Phelan_structure),
an odd bird made from alternating two different polyhedra.
Like blowing bubbles to check the isoperimetric inequality,
carefully designed soap bubble experiments reproduce the Weaire-Phelan structure.
...
I would also like to thank the students for their participation and feedback.

#### References

- ["**NP-complete problems and physical reality**"](https://www.scottaaronson.com/papers/npcomplete.pdf)
(2005), Scott Aaronson.
- [*Kinetics of Material*](https://onlinelibrary.wiley.com/doi/book/10.1002/0471749311)
