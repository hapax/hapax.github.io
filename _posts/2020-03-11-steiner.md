...
an edge.
We'll focus on what happens a vertex.
...
The solid angles [are](https://en.wikipedia.org/wiki/Platonic_solid#Angles)
...
For simplicity, we'll assume that only other vertices meet at a
corner.
...
Conclude that the cube forms the basis of a tessellation.
</span>

Even if we allow solids to slide around so that vertices
touch edges or faces, one can show that the cube is still the only
regular polyhedron which can fill space.
Let's see if it could possibly describe a foam.

<span style="padding-left: 20px; display:block">
(g) Using Exercise 20 or Plateau's laws, prove an infinite foam cannot be cubic.
</span>
...
working out which ones can tessellate is hard. This time, we'll just skip
