...
Steiner trees in the plane, and other closely related variants, pop in
...
Apart from rail networks, we won't explore these problems, since most
involve variants of the problem in the plane, but many of the insights
carry over.
...
*Note for computational complexity geeks.* This makes the mathematical solution of the minimal network
effectively impossible to compute.
The technical term is
[**NP-hard**](https://en.wikipedia.org/wiki/NP-hardness), meaning
roughly that (assuming that [most important unsolved conjecture](https://en.wikipedia.org/wiki/P_versus_NP_problem)
in computer science is true) there is no quick way to find the exact
minimal network.
It will always take a ridonkulously long time!
But, as we will touch on below, it turns out to be easy to
*approximate* Steiner trees, so (once again for complexity geeks) it lies in
