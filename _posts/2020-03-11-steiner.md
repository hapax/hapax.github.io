...
<span style="padding-left: 20px; display:block">
(e) Verify that only the cube has a vertex solid
angle ($\Theta = \pi/2$) taking the form $\Theta = 4\pi/n$ for some whole
number $n$.
</span>

<span style="padding-left: 20px; display:block">
(f) Using similar arguments to the plane, show that only the cube can
tessellate space. As before, deduce how many cubes must meet at a
vertex, and show they can slide.
</span>

<span style="padding-left: 20px; display:block">
(g) Could an infinite foam assume this form?
