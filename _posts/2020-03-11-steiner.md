...
In this exercise, we will try to determine which regular polyhedra can
fill space.

As a warm-up, let's consider the plane, which can be tiled in three
...
In this case, there are only five regular polyhedra, [the Platonic solids](https://en.wikipedia.org/wiki/Platonic_solid):
cube,
...
<span style="padding-left: 20px; display:block">
(c) Argue that, for a Platonic solid to tessellate "vertex only", it
must have vertex angle $\Omega = 4\pi/n$ for some integer $n$.
</span>

<span style="padding-left: 20px; display:block">
(d) Using a calculator or symbolic algebra package, show that only
...
<span style="padding-left: 20px; display:block">
(e) Suppose we tessellate space using a cell with $f$ faces, each with
$e$ edges.
