...
Take a cross-section $A$ of a three-dimensional foam which "cuts
through" every face and every Plateau border, giving a network where
edges correspond to faces of the soap film and points to Plateau borders.

<span style="padding-left: 20px; display:block">
(a) Show that if $A$ is perpendicular to a Plateau border, edges will
...
<span style="padding-left: 20px; display:block">
(c) Does this affect the result $D_\text{hex} = 6 + E_b$ from <a href="#sec-4-1">above</a>?
</span>
