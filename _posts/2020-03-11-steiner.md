...
We will do this in the form of a sequence of exercises.
...
In the plane, there are three tessellations by regular
polygons: squares, triangles, and hexagons.
As a warm-up for three-dimensional space, let's see why.

<span style="padding-left: 20px; display:block">
(b) Prove that internal angle of a regular $d$-gon is
</span>
...
Assume that the regular polygons are tiled so they meet at a vertex.
  The angles at a vertex must add up to $360^\circ = 2\pi$ (in
  radians).

<span style="padding-left: 20px; display:block">
(c) Explain why we must have $2\pi = n\theta$ for some whole number
$n$.
	Conclude, using part (b), that the square, triangle and hexagon
	are the only possibilities, and work out $n$ (the number meeting
	at a vertex) for each.
</span>

<span style="padding-left: 20px; display:block">
(d) You might think that we could "slide" regular polygons around, so that
a vertex one on polygon meets an edge of another.
Argue that, in this case, $\pi = \theta n$ for some $n$, and conclude
that only the square can "slide".
Construct a tessellation which shows this is the case.
</span>

Unlike the plane, where there are infinitely many regular polygons, in
three-dimensional space there are the
[five Platonic solids](https://en.wikipedia.org/wiki/Platonic_solid):
the cube,
tetrahedron, octahedron, dodecahedron and icosahedron.
Instead of plane angles, they have
[*solid angles*](https://en.wikipedia.org/wiki/Solid_angle).
Instead of $2\pi$, the circumference of a unit radius circle, the
solid angle around a point is $4\pi$, the surface area of a unit
radius sphere.
A somewhat trickier calculation shows that the
[solid angles](https://en.wikipedia.org/wiki/Platonic_solid#Angles)
$\Theta$ at the vertex of each Platonic solid is, respectively,

$$
\frac{\pi}{2}, \quad \cos^{-1}(23/27), \quad 4\sin^{-1}(1/3), \quad,
\pi - \tan^{-1}(2/11), \quad 2\pi - 5\sin^{-1}(2/3).
$$

Only the cube (solid angle $\Theta = \pi/2$) takes the form $4\pi/n$
for some whole number $n$.

<span style="padding-left: 20px; display:block">
(e) Using similar arguments to the plane, show that only the cube can
tessellate space. As before, deduce how many cubes must meet at a
vertex, and show they can slide.
</span>

<span style="padding-left: 20px; display:block">
(f) In a cubic tessellation, each cell has six square faces.
Could an infinite foam assume this form?
*Hint*. Consider Exercise 20(d) and part (a).
</span>
...
There are $26$ of these altogether, so I won't try to list them all!
...
The cells of our new tessellations are as follows:
...
If so, then this is the structure four-dimensional bees would store
their honey in!
...
which solve deep mathematical problems.
