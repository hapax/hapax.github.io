...
The rules are called *Plateau's laws*, and were proposed by Belgian
mathematician [Joseph Plateau](https://en.wikipedia.org/wiki/Joseph_Plateau)
(1801-1883) after spending a long time playing with soap bubbles. Here
are the first three:

1. The faces in a soap film are smooth.
2. Any face has constant mean curvature.
3. Faces always meet three at a time, separated by angles of
   $120^\circ$. The shared edge at which faces meet is called *Plateau borders*.

Constant mean curvature is the requirement that any (straight) slice
through the surface looks like either like a straight line or the arc of a circle.

We now consider a cross-section $A$ of a three-dimensional foam, which
we assume "cuts through" every face and every Plateau border,
i.e. $A$ is a network where edges correspond to faces of the soap
film, and points to Plateau borders.

<span style="padding-left: 20px; display:block">
(a) Show that if $A$ is perpendicular to a Plateau border, edges
meet at the corresponding point at angles of $120^\circ$.
</span>

<span style="padding-left: 20px; display:block">
(b) If $A$ is *not* perpendicular to a Plateau border, prove that
edges do not meet at $120^\circ$.
</span>

<span style="padding-left: 20px; display:block">
(c) Does the result $D_\text{hex} = 6 + E_b$ from <a
href="#sec-4-1">above</a> change?
</span>
