...
for *any* $k$, where $\gtrsim$ means "for all suitably large $n$", and
$\alpha$ is just a constant.
...
This is a very different question! It asks for a *single* point such
that the sum of lengths to that point is minimal, rather than a
connected network of minimal length.
The two problems only agree for $n = 3$.

Fermat, Torricelli, and Steiner were interested in geometry.
