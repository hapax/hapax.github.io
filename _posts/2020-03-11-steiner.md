...
statement).
It is clear from experiment --- i.e. blowing bubbles
which enclose precisely the volume of air you blow into them --- that
nature opts for this solution.

This suggests that the sphere is not only locally minimal, but the
*globally* minimal surface enclosing a fixed volume, a mathematical
result called the *isoperimetric inequality*.
