...
different configurations of four cities, but moved on.
...
of the triangle.
Finding the point is still a quite involved exercise in trigonometry,
and to make life easier, we will assume a few standard theorems along
the way.

We will give the explicit construction, and then check that it works.
Consider a triangle with sides $A, B, C$, displayed in Figure 18.
On each side, attach equilateral triangles (green, red, yellow) with
the same length.
...
<figure>
    <div style="text-align:center"><img src
    ="/images/posts/steiner29.png" width="80%"/>
...
We want to prove these circles intersect at $f$ using the inscribed
angle theorem.

<span style="padding-left: 20px; display:block">
(a) Show that $\triangle baf$ is congruent to $\triangle bcf$.
</span>

<span style="padding-left: 20px; display:block">
(b) Using part (a), deduce that
</span>
...
<span style="padding-left: 20px; display:block">
(c) Conclude from
[Ptolemy's theorem](https://en.wikipedia.org/wiki/Ptolemy%27s_theorem)
that $f$ lies on the circle through $a, b, c$.
Repeating the reasoning for each side, $f$ lies on the intersection of circles.
</span>

<span style="padding-left: 20px; display:block">
(d) The angle $\angle cba = 60^\circ$, since the triangle is equilateral.
Using the
[inscribed angle theorem](https://en.wikipedia.org/wiki/Inscribed_angle),
show that $\angle cfb = 120^\circ$.
Applying this to each triangle gives our result!
</span>
...
In real foams, like the photo below, bubble walls are curved rather
than straight.
...
But to minimise near a junction, we only care about the nearby part of the
bubble walls.
If we zoom in enough on the junction, the nearby bubble walls look approximately straight!
Our minimisation argument still applies, showing that even curved walls
intersect three at a time at angles of $120^\circ$.
You can check this is true for the image above.
It follows that a two-dimensional foam with curved walls will almost
entirely consist of hexagonal cells.
...
We can pursue this obervation further, but for our purposes,
we'll just consider the traditional division of computers into hardware and software.
...
<figure>
    ...
    ="/images/posts/steiner25.png" width="58%"/>
		    <figcaption><i>Figure 26. German cities to be connected by rail.</i></figcaption>
	</div>
	</figure>
...
so I would also like to thank the students for their participation and feedback.
