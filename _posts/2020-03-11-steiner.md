...
in Exercise 19 and 20. Exercise 18 introduces a technical result about
faces and edges we need to study infinite foams.
...
*Exercise 19.* In the previous exercise, we introduced a relation
between the *average* number of faces a cell, edges of a cell, and
edges of a face.
Since we are discussing averages, they continue to make sense, even if
the foam is infinite!
Suppose that each cell looks the same, so we are limited to
space-filling tessellations by a single polyhedron, in the same that the <a
href="#sec-4-2">hexagonal lattice</a> fills the plane.
We will check which of these tessellations are consistent with
Exercise 18.

<span style="padding-left: 20px; display:block">
(a) Suppose we tessellate space using a cell with $f$ faces, each with
$e$ edges.
Show that the cell has $E = fe/2$ edges.
Generalise to $f_1$ faces with $e_1$ edges each, and $f_2$ faces with
$e_2$ edges.
</span>

<span style="padding-left: 20px; display:block">
(b) In the plane, there are three tessellations by *regular
polygons*: squares, triangles, and hexagons.
Unlike the plane, where there are infinitely many regular polygons, in
three-dimensional space there are five, the
[Platonic solids](https://en.wikipedia.org/wiki/Platonic_solid): the cube,
tetrahedron, octahedron, dodecahedron and icosahedron.
Show that only the cube can tessellate space.
</span>

<span style="padding-left: 20px; display:block">
(c) In a cubic tessellation, each cell has six square faces.
Show that $$
Could an infinite foam assume this form?
*Hint*. Consider Exercise 18(d) and part (a).
</span>

Let's expand our horizons beyond Platonic solids to the slightly
more general
[Archimedean](https://en.wikipedia.org/wiki/Archimedean_solid) and
[Catalan solids](https://en.wikipedia.org/wiki/Catalan_solid).
There are $26$ of these altogether, so I won't try to describe them!
Two of them can be spammed to tessellate space, the
...
Although this takes us beyond the Archimedean/Catalan solids, note
that if we extrude each face of the rhombic dodecahedron to a pyramidal point, we get
the *stellated rhombic dodecahedron*, aka ["Escher's solid"](https://en.wikipedia.org/wiki/First_stellation_of_the_rhombic_dodecahedron),
named after Escher's marvellous lithograph [*Waterfall*](https://en.wikipedia.org/wiki/Waterfall_(M._C._Escher)).
This stellation can also tessellate space, so I think of it as a  "testellation".

<span style="padding-left: 20px; display:block">
(d) The cells of our new tessellations are as follows:
- the rhombic dodecahedron, with twelve rhombic faces;
- Escher's solid, with $48$ triangular faces (each rhombus
  becomes a square-based pyramid); and
- the truncated octahedron, with eight hexagonal faces and six
squares.

Only one of these is consistent with Exercise 18, and can therefore
describe a foam. Which one is it?
</span>
...
In Exercise 19, you found a space-filling tessellation which could in
principle describe an infinite three-dimensional foam, consisting of
cells of uniform volume.
[Lord Kelvin](https://en.wikipedia.org/wiki/William_Thomson,_1st_Baron_Kelvin)
(1824-1907) conjectured that this was the most efficient way to
separate equal volume cells, so it is called the *Kelvin structure* in
his honour.
This is essentially just the three-dimensional version of the <a
href="#sec-4-2">honeycomb conjecture</a> we stated above.

But there are reasons to be cautious.
Our little catalogue of tessellations suggests that there are more
possibilities in three dimensions than in two, so perhaps we should
not be so quick to dismiss the possibility that some other
configuration, with an odd shape, or two or more cell types of the
same volume, beats the Kelvin structure for efficiency.
And indeed, in 1993, more than 100 years after Kelvin made his
conjecture, Weaire and Phelan found a way to beat the Kelvin structure
by a miniscule $0.3 \%$.
It is unknown whether this is optimal however!

---

*Exercise 20.* The
 [Weaire-Phelan](https://en.wikipedia.org/wiki/Weaire%E2%80%93Phelan_structure)
 structure alternates two different cells of equal volume:
...
Check this is consistent with Exercise 18(d) and (e).
...
it can be found in certain naturally occurring crystals.
Once again, the laws of physics lead to "natural computers"
which solve hard mathematical problems.

It's wonderful that something as ordinary as a soap bubble could so
powerfully embody the deep interrelationship between physics,
...
As natural computers, there is more in *playing* with them than generally imagined as well.
