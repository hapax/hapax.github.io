...
**March 23, 2021.** *We turn the old joke about interesting numbers into a
  proof that most real numbers are indescribably boring. This implies
  that there is no explicit well-ordering of the real
  numbers. Assuming the axiom of choice, we show that all real numbers
  are relatively interesting, using the corresponding
  (non-constructive) well-ordering of the reals.*
...
to the Berry paradox, so one might worry that we subtly fail to have
an explicit, finite description for the same reasons.
