...
If it didn't, then the *minimum* we used to get our contradiction
...
boring rationals, we can define the "smallest" boring number as
...
going to explain why it might exist anyway, depending on which axioms of set theory we use!
...
Most literally cannot be talked about.
...
that $b \in B_\mathbb{R}$ is the smallest minimizer of $f$, i.e.
...
But we can always make the *existence* of such a rule --- describable
...
*axiom of choice*, which is logically equivalent, as we will argue
...
The overarching theme of this post is how much mileage we can get
from a bad joke.
...
interesting, even if we can't talk about them.

#### Acknowledgments

As usual, thanks to J.A. for the discussion which led to this
post, and also for proposing an elegant mapping analogous to unicoding.
...
Since lim-definability is closed under going meta, as is "finite",
this is *now a definition at the same level*.
Option (b) is no longer available to us, so only option (a)
remains, and it follows that, like the joke that began it all, *all
positive integers are finitely lim-definable*.
This is of course obviously true.

Our argument against an explicit well-ordering is very closely related
...
explicit well-ordering in particular.
...
for any ordinal $\alpha$. The resulting set of
representatives, labelled by ordinals, is
...
point was that the argument looks pretty similar to the previous one!
