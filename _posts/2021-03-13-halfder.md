...
**March 13, 2021.** *Can you take half a derivative? Or π derivatives?
  Or even i derivatives? It turns out the answer is yes, and there are
  two reasonably simple, but seemingly different, ways to do it. I
  show they are actually equivalent!*
...
I'm not sure this has any applications, but it's cute.
I invite the interested reader to take $\pi$ derivatives of sine.
...
We'll actually define it in the last section, but for the moment, the
...
and (b) is defined for non-integer values also. I like to think of it as the
"fractorial" because it makes sense for fractions! In addition to
delightfully bad puns, we can use the Gamma function to write
...
queasy. But there is a trick here which is almost rigorous. It turns out that for any $z$,
...
It also gves the sneaky result $\Gamma(0) = (-1)\Gamma(-1)$. Both $\Gamma(0)$ and
...
derivatives defined using exponentials to *derive* the fractorial
approach, i.e. the Gamma function version.
And in order to this, we first have to grit our teeth and define the
...
You can find proofs of the functional equation and so on
[elsewhere](https://en.wikipedia.org/wiki/Gamma_function).
We won't concern ourselves with proving these things.
...
$e^{\pm \pi i \alpha}$ for the terms they multiply so they perfectly
cancel!
The result is that our funny factor is just unity:
