...
or an extension to reciprocal powers called the *Laurent series*:

$$
\sum_{k = -\infty}^\infty b_k x^k,
$$
...
\frac{\Gamma(0)}{\Gamma(-1)} \frac{1}{x^2}.
$$

Both the numerator and the denominator blow up. But there is a trick
here to get the right answer: it turns out that for any $z$,

$$
\Gamma(1 + z) = z\Gamma(z).
$$

This gives the usual relation for factorials,

$$
k! = \Gamma(k + 1) = k\Gamma(k) = k \cdot (k - 1)!,
$$

but also the sneaky $\Gamma(0) = (-1)\Gamma(-1)$. Both blow up of
course, but the $\Gamma(-1)$ terms cancel, leaving $-1/x^2$ as required.
