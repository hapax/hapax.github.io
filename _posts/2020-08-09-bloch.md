...
We would like to interpret $|\alpha|^2$ and $|\beta|^2$ as these
probabilities, but for this to make sense, they must satisfy the normalization condition
$|\alpha|^2 + |\beta|^2 = 1$.
Writing this out in terms of the real and imaginary components
...
In other words, the Hopf map takes us from a point on the 3-sphere to the
point its phase circle is "pasted" onto the 2-sphere.
Since we can choose to embed the circle $\mathbb{S}^1$ into any class
$e^{i\mathbb{S}^1}(\alpha,\beta) \subseteq \mathbb{S}^3$, we write the
full fibration as
...
The state mappings are complicated for reasons I discuss below, but the fibrations generalise in a beautiful way from the 3-sphere case.
...
is much messier. The reason is simple: global phase ambiguity is
still a circle!
The space of two- and three-qubit states is $7-1 = 6$ and $15-1=14$
respectively, where we subtract one to account for the one-dimensional circle of
equivalent states we are identifying.
This means the pure states are split between the base and the fibre, e.g. the $\mathbb{S}^3$ which is
