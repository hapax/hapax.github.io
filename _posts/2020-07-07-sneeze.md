...
When it moves fast, there is a different physics at play, and
...
$$
R(r) = \frac{m}{\pi \rho r^2} \left[2\log\left(\frac{2 \rho r v_0}{\eta}\right) + \frac{1}{12}\right].
$$

(b) By setting $R'= 0$, find the maximum range and corresponding
droplet size.
Consult the literature on droplet size distribution and comment on the
WHO guidelines.

(c) (*For the author.*) Redo the analysis in two dimensions. Note that
there is no analytic solution for quadratic drag for general firing
angle, so you will be reduced to numerics.
