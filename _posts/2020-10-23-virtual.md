...
can the interactions in both directions.
...
and to explore the relation to the MTML bound.
Another question is whether we can say anything about about the
change in expected particle for other virtual particles mediating the
transition.
For instance, in the simple particle-antiparticle example above, what
can we say about the $a$ and $b$ content after $\tau_c$?
But the most intriguing problem is to connect this non-relativistic
inequality, and the toy models, to the rigorous interpretation
of virtual particles in quantum field theory.
I expect that we can extend the cubic particle-antiparticle
example to a Fock space of momentum modes for a charged scalar
field. I leave this as an exercise for the interested reader!
